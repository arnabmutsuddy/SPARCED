...
parser.add_argument('--submodel', help='input submodel name', default='submodel_1')
...
os.mkdir(os.path.join('submodels',submodel_name))
os.mkdir(os.path.join('submodels',submodel_name,'input_files'))

#%% make a random smaller subset of ratelaws input file
#ratelaw_original = np.array([np.array(line.strip().split("\t")) for line in open(os.path.join(input_data_folder,'Ratelaws.txt'))])
import pandas as pd

ratelaw_original = pd.read_csv('input_files/Ratelaws.txt',sep='\t',header=0,index_col=0)

#%%

ratelaw_submodel = ratelaw_original.iloc[np.unique(np.random.randint(1,(len(ratelaw_original)-1),np.random.randint(100,200)))]

ratelaw_submodel.to_csv(os.path.join('submodels',submodel_name,'input_files','Ratelaws.txt'), sep='\t', header=True, index=True)

#%%

stoicmat_original = pd.read_csv('input_files/StoicMat.txt', sep='\t', header=0, index_col=0)

stoicmat_sub = stoicmat_original.loc[:,ratelaw_submodel.index]

species_rp = np.array(stoicmat_sub.index[np.nonzero(stoicmat_sub.sum(axis=1).values)[0]])

#%%
# Antimony model name and text
fileModel = open(os.path.join('submodels',submodel_name,str(submodel_name+'.txt')),'w') # file name
fileModel.write("# PanCancer Model by Birtwistle Lab \n") # some explanation
fileModel.write(str(f'model {submodel_name}()\n')) # model name


# SBML model we want to import
sbml_file = str(submodel_name+'.xml')
...
ratelaw_compartments = np.unique([ratelaw_data[i][0] for i in range(len(ratelaw_data))])

if ~np.isin(ratelaw_compartments,compartment_sheet[1:,0]).any():
    print("ERROR: Unknown compartment in Ratelaws")
    exit(1)
...
for rowNum, ratelaw in enumerate(ratelaw_data):
    ...
    if "k" not in ratelaw[1]:
        ...
        if len(params) == 1:
            ...
            mrna = re.search(r"mrna_[A-Za-z0-9]+", ratelaw[1])
            for p in params:
                paramnames.append("k"+str(rowNum+1)+"_"+str(j))
                paramvals.append(float(ratelaw[j+1]))
