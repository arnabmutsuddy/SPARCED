...
timecourse('RasT_BRaf_BRaf', x_s = xoutS_all,tout_all= tout_all)

#xoutS_all[:,list(species_all).index('E')]

#%%

timecourse_mrna('CDK4')
...
params_check = pd.Series(data=model_module.getModel().getFixedParameters(), index=model.getFixedParameterIds())
...
#%%
mrna_stable = pd.read_csv('mrna_stable.csv', sep='\t', index_col=0, squeeze=True)
...
condsSDF_read = pd.read_csv('new_odes_nostim.csv', sep='\t', index_col=0, header=0)
