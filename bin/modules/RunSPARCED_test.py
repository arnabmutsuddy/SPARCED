...
def RunSPARCED_test(flagD,th,spdata,genedata,Vn,Vc,model,input_data_folder):
    ...
    vTC_all = np.zeros(shape=(NSteps,int(len(genedata)/3)))
    vTCd_all = np.zeros(shape=(NSteps,int(len(genedata)/3)))
    Nb_all = np.zeros(shape=(NSteps,int(len(genedata)/3)))
    Nd_all = np.zeros(shape=(NSteps,int(len(genedata)/3)))
    hills_all = np.zeros(shape=(NSteps,int(len(genedata)/3)))
    
    TFa_all = np.zeros(shape=(NSteps,np.shape(TARs0)[0],np.shape(TARs0)[1]))
    TFr_all = np.zeros(shape=(NSteps,np.shape(TARs0)[0],np.shape(TARs0)[1]))
    
    
    for qq in range(NSteps):
        genedata,xmN_nM,AllGenesVec,vTC,vTCd,Nb,Nd,hills,TFa,TFr = SGEmodule_test(flagD,ts,xoutG_all[qq,:],xoutS_all[qq,:],Vn,Vc,kTCmaxs,kTCleak,kTCd,AllGenesVec,GenePositionMatrix,kGin_1,kGac_1,tcnas,tck50as,tcnrs,tck50rs,spIDs,mrna_IDs_sge)
    ...
    return xoutS_all, xoutG_all, xoutObs_all, tout_all, vTC_all, vTCd_all, Nb_all, Nd_all, hills_all,TFa_all,TFr_all